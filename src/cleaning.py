...
df["MonthlyIncome(NGN)"]=(df["MonthlyIncome(NGN)"].fillna(df.groupby(["Sector","BackupPower"])["MonthlyIncome(NGN)"].transform("mean"))).round(1)
df["MonthlyBill(NGN)"]=df["MonthlyBill(NGN)"].fillna(df.groupby(["State","Sector"])["MonthlyBill(NGN)"].transform("mean")).round(1)
...
df["ConsumptionCategory"]=df["ConsumptionCategory"].astype(str).str.capitalize().str.strip()
consumption_corrector={"Heavy":"High","Extreme":"Very high","Very heavy":"Very high",
                       "H":"High","Vh":"Very high","Average":"Moderate","M":"Moderate","Medium":"Moderate",
                       "L":"Low","Minimal":"Low"}
df["ConsumptionCategory"]=df["ConsumptionCategory"].replace(consumption_corrector)
# saving cleaned data
df.to_csv("Cleaning.csv",index=False)
# feature engineering
df["MonthlyIncome_log"]=np.log1p(df["MonthlyBill(NGN)"]).round(4)
df["MonthlyBill_log"]=np.log1p(df["MonthlyBill(NGN)"]).round(4)
df["GeneratorFuelCost_log"]=np.log1p(df["GeneratorFuelCost(NGN)"])
df["MonthlyConsumption_log"]=np.log1p(df["MonthlyConsumption(kWh)"])
df=df.drop(columns=["MonthlyIncome(NGN)","HouseholdID","MonthlyBill(NGN)",
                    "GeneratorFuelCost(NGN)","MonthlyConsumption(kWh)"])
df["consumption_rate"]=(df["MonthlyConsumption_log"]/df["NumOccupants"]).round(2)
df["House_comfort"]=(df["MonthlyBill_log"]/(df["NumRooms"]+df["NumAppliances"])).round(3)
df["usage_rate"]=(df["MonthlyIncome_log"]/(df["MonthlyBill_log"]+df["GeneratorFuelCost_log"])).round(3)
df["appliance_consumption"]=(df["HoursPowerDaily"]/(df["NumRooms"]+df["NumAppliances"])).round(3)
df["Hourly_rate"]=(df["MonthlyConsumption_log"]/df["HoursPowerDaily"]).round(3)
df.to_csv("Cleaning&Engineering.csv", index=False)
print(df["NumAppliances"].describe())
